...
#ifdef DPA_ENABLE
    dpa_rx_ring_count = min_t(u32, dpa_num_rx_descs, IGB_MAX_RXD);
    dpa_rx_ring_count = max_t(u16, dpa_rx_ring_count, IGB_MIN_RXD);
    adapter->rx_ring_count = ALIGN(dpa_rx_ring_count, REQ_RX_DESCRIPTOR_MULTIPLE);
#else
...
#ifdef DPA_ENABLE
    if (!rss_queues) {
        rss_queues = IGB_MAX_RX_QUEUES;
    } else {
        rss_queues = min_t(u32, IGB_MAX_RX_QUEUES, rss_queues);
    }
    adapter->rss_queues = min_t(u32, rss_queues, num_online_cpus());
#else
...
#ifdef DPA_ENABLE
    if (igb_dpa_conf_rx_ring(rx_ring)) {
        return;
    }
#endif
