...
#ifdef DPA_ENABLE
#undef DRV_MODULE_VERSION
#define DRV_MODULE_VERSION  "2.1.11 -DPA"
#endif
...
#ifdef DPA_ENABLE
static int rss_queues = 0;
module_param(rss_queues, int, 0);
MODULE_PARM_DESC(rss_queues, "Number of Receive-Side Scaling Descriptor Queues");
#endif
...
#ifdef DPA_ENABLE
    ring_id = bnapi-(bp->bnx2_napi);
    if (dpa_rx_irq(bp->dev, ring_id, &work_done)) {
        return 1;
    }
#endif
...
bnx2_init_fw_cap(struct bnx2 *bp)
{
    ...
    if ((val & BNX2_FW_CAP_SIGNATURE_MASK) != BNX2_FW_CAP_SIGNATURE)
        return;
#ifdef DPA_ENABLE
    if ((val & BNX2_FW_CAP_CAN_KEEP_VLAN) == BNX2_FW_CAP_CAN_KEEP_VLAN || !dpa_flag_vlan_strip)
#else
...
#ifdef DPA_ENABLE
    if (bnx2_dpa_conf_rx_ring(bp, rxr, ring_num)) {
        goto dpa_next;
    }
#endif
...
dpa_next :
    ...
    REG_WR16(bp, rxr->rx_bidx_addr, rxr->rx_prod);
    REG_WR(bp, rxr->rx_bseq_addr, rxr->rx_prod_bseq);
}
...
#ifdef DPA_ENABLE
        snprintf(bp->irq_tbl[i].name, len, "%s-TxRx-%d", dev->name, i);
#else
...
#ifdef DPA_ENABLE
    if (rss_queues > 0) {
        msix_vecs = min(msix_vecs, rss_queues);
    }
#endif
...
#ifdef DPA_ENABLE
    if (ering->rx_pending != bp->rx_ring_size) {
        dpa_error("Changing rx_ring_count is not supported by DPA-aware driver.");
        return -EINVAL;
    }
#endif
...
#ifdef DPA_ENABLE
    dpa_error("Changing MTU is not supported by DPA-aware driver.");
    return -EINVAL;
#endif
...
bnx2_init_board(struct pci_dev *pdev, struct net_device *dev)
{
    ...
    int err;
#ifdef DPA_ENABLE
    int dpa_rx_ring_count = MAX_TOTAL_RX_DESC_CNT;
#endif
...
#ifdef DPA_ENABLE
    dpa_rx_ring_count = min_t(u32, dpa_num_rx_descs, MAX_TOTAL_RX_DESC_CNT);
    dpa_rx_ring_count = (dpa_rx_ring_count % MAX_RX_DESC_CNT == 0) ?
        dpa_rx_ring_count : ((dpa_rx_ring_count / MAX_RX_DESC_CNT) + 1) * MAX_RX_DESC_CNT;
    bnx2_set_rx_ring_size(bp, dpa_rx_ring_count);
#else
