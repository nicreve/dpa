...
#ifdef DPA_ENABLE
static unsigned int rss_queues = 0;
module_param(rss_queues, uint, S_IRUGO);
...
#ifdef DPA_ENABLE
    if (ixgbe_dpa_conf_rx_ring(rx_ring)) {
        return;
    }
#endif
    while (cleaned_count--) {
...
#ifdef DPA_ENABLE
    if (dpa_rx_irq(rx_ring->netdev, rx_ring->queue_index, work_done)) {
        return;
    }
#endif
...
{
    struct ixgbe_hw *hw = &adapter->hw;
    /* 更改Hash key为循环16bit，支持将同一流的上行与下行报文hash至同一队列 */
#ifdef DPA_ENABLE
    static const u32 seed[10] = { 0x5A6D5A6D, 0x5A6D5A6D, 0x5A6D5A6D,
              0x5A6D5A6D, 0x5A6D5A6D, 0x5A6D5A6D, 0x5A6D5A6D,
              0x5A6D5A6D, 0x5A6D5A6D, 0x5A6D5A6D};
#else
...
#ifdef DPA_ENABLE
    if (!rss_queues) {
        rss_queues = IXGBE_MAX_RSS_INDICES;
    } else {
        rss_queues = min(IXGBE_MAX_RSS_INDICES, (int)rss_queues);
    }
    rss = min(rss_queues, num_online_cpus());
#else
...
#ifdef DPA_ENABLE
    dpa_rx_ring_count = min_t(u32, dpa_num_rx_descs, IXGBE_MAX_RXD);
    dpa_rx_ring_count = max_t(u16, dpa_rx_ring_count, IXGBE_MIN_RXD);
    adapter->rx_ring_count = ALIGN(dpa_rx_ring_count, IXGBE_REQ_RX_DESCRIPTOR_MULTIPLE);
#else
